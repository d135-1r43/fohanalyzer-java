...
/**
 * Runs {@link Settings} against a throwaway {@link Preferences} node rather than the real
 * user store, so the suite neither reads nor leaves behind anyone's actual calibration.
 * Only {@code javafx.base} properties are involved — no toolkit, so this stays a plain
 * unit test.
 */
class SettingsTest {
    ...
    void tearDown() throws Exception {
        ...
        node.removeNode();
        parent.flush(); // the removal is only persistent once an ancestor is flushed
    }
    ...
    void savedLiveInputIsDroppedWhenTheInterfaceIsGone() {
        ...
        AppState st = new AppState(); // nothing enumerated: different venue, device absent
        settings.bind(st);
    ...
    void resetRestoresDefaultsAndEmptiesTheStore() throws Exception {
        ...
        assertEquals(defaults.frac.get(), st.frac.get());
        assertEquals(defaults.markerSource.get(), st.markerSource.get());
        // Applying the defaults fires the persist listeners, so this also pins down that the
        // store is wiped afterwards rather than left holding explicit copies of the defaults.
