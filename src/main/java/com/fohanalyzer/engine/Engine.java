...
/**
 * Pure frequency math and the signal simulator. Direct port of {@code src/lib/engine.js}.
...
public final class Engine {
    ...
    public static double freqNorm(double f) { return (log2(f) - LOGMIN) / LOGSPAN; }

    /** Inverse of {@link #freqNorm}. */
    public static double normFreq(double n) { return Math.pow(2, LOGMIN + n * LOGSPAN); }

    /** Log-spaced 1/{@code frac}-octave band centres anchored at 1 kHz, within FMIN..FMAX. */
    public static double[] bandCenters(int frac) {
        ...
        for (int k = kmin; k <= kmax; k++) out.add(1000 * Math.pow(r, k));
    ...
    /**
     * Generate one frame of the two simulated traces at time {@code t} (seconds).
     * {@code ring} may be {@code null} (treated as inactive). Direct port of {@code sample()}.
     */
