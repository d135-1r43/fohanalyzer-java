...
/**
 * Build tool: renders the app icon from the in-app {@link Logo} artwork, so the icon cannot
 * drift from the logo in the header. Writes a macOS {@code .iconset} directory that
 * {@code iconutil} turns into an {@code .icns}.
 *
 * <p>Each size is rendered from the vector artwork rather than downscaled from one large
 * bitmap — the bars are thin, and resampling them turns the small sizes to mush.
 *
 * <p>Run via {@code scripts/package-mac.sh}, or directly:
...
public class IconRenderer extends Application {
    ...
    /** {@code {pixels, filename}} — the set {@code iconutil} expects for a macOS icon. */
    ...
    /** Artwork inset inside the canvas, so the icon is not full-bleed against the Dock. */
    ...
    private static void write(int px, File target) throws Exception {
        ...
        // A Scene is needed for layout, but never shown — this renders offscreen.
