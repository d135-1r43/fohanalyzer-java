 ...
 * {@link GraphicsContext}. The plot is laid out with the same padding and dB/freq
...
public final class Draw {
    ...
    private static final double[] FREQ_MAJ_F =
        {20, 50, 100, 200, 500, 1000, 2000, 5000, 10000, 20000};
    private static final String[] FREQ_MAJ_L =
        {"20", "50", "100", "200", "500", "1k", "2k", "5k", "10k", "20k"};
    private static final double[] FREQ_MIN =
        {30, 40, 70, 300, 400, 700, 3000, 4000, 7000, 15000};
    ...
    private static void roundRect(GraphicsContext g, double x, double y, double w, double h, double r) {
        // GraphicsContext fillRoundRect/strokeRoundRect take arc width/height (= 2r).
        // Callers invoke fill/stroke directly; this helper just centralises the radius.
    ...
    public static void drawGrid(GraphicsContext g, double W, double plotW, double plotH) {
        g.setLineWidth(1);
    ...
    public static BarLayout computeBarLayout(double[] centers, double plotW) {
        ...
        for (int i = 0; i < n; i++) xs[i] = fX(centers[i], plotW);
    ...
    public static void drawTransferCurve(GraphicsContext g, double[] centers, float[] dispMic,
                                         float[] dispSolo, double W, double plotW, double plotH) {
        double TF = 24, midY = PAD_T + plotH / 2, half = (plotH / 2) * 0.8;
        g.setStroke(Color.rgb(183, 148, 246, 0.28));
        g.setLineDashes(6, 6);
        g.strokeLine(PAD_L, midY, W - PAD_R, midY);
        g.setLineDashes(null);
        g.beginPath();
        for (int i = 0; i < centers.length; i++) {
            double d = dispMic[i] - dispSolo[i];
            double x = fX(centers[i], plotW);
            double y = midY - Math.max(-TF, Math.min(TF, d)) / TF * half;
            if (i == 0) g.moveTo(x, y); else g.lineTo(x, y);
        }
        g.setLineWidth(2.5);
    ...
    public static void drawTransferLabels(GraphicsContext g, double W, double plotW, double plotH) {
        double TF = 24, midY = PAD_T + plotH / 2, half = (plotH / 2) * 0.8;
        ...
        for (int i = 0; i < ds.length; i++) {
            double y = midY - ds[i] / TF * half;
            g.setFill(Color.rgb(183, 148, 246, 0.7));
            g.fillText(ls[i], W - PAD_R - 6, y);
        }
    ...
    public static void drawPeakMarkers(GraphicsContext g, float[] src, double[] centers,
                                       double plotW, double plotH, double W, Color col) {
        int n = src.length;
        ...
        for (int k = 0; k < Math.min(3, peaks.size()); k++) {
            ...
            double bx = x - tw / 2 - 6;
            bx = Math.max(PAD_L + 2, Math.min(bx, W - PAD_R - tw - 12));
    ...
    public static void drawLocateLine(GraphicsContext g, double locateFreq, double W,
        ...
        double bx = x - tw / 2 - 6;
        bx = Math.max(PAD_L, Math.min(bx, W - PAD_R - tw - 12));
    ...
    public static void drawHoverTooltip(GraphicsContext g, Mouse mouse, double[] centers, double[] xs,
                                        float[] dispMic, float[] dispSolo, double W, double plotW,
                                        double plotH, boolean micOn, boolean soloOn) {
        double mx = mouse.x(), my = mouse.y();
        if (mx <= PAD_L || mx >= W - PAD_R || my <= PAD_T || my >= PAD_T + plotH) return;
        int n = centers.length;
        ...
        g.setFont(font);
        double bw = 0;
        for (Line l : lines) bw = Math.max(bw, Fonts.width(font, l.t()));
        bw += 18;
        double bh = lines.size() * 16 + 10;
        double bx = cx + 12;
        if (bx + bw > W - PAD_R) bx = cx - bw - 12;
        double by = my - bh - 10;
    ...
    public static void drawZoneStrip(GraphicsContext g, double plotW, double plotH, double W, double H) {
