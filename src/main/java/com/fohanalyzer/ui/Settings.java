...
/**
 * Persists the parts of {@link AppState} an engineer would resent re-entering: SPL
 * calibration, the selected inputs, and the analysis/view options.
 *
 * <p>Backed by {@link Preferences}, so there is no dependency and no file format of our own
 * to version. Stored values are validated on the way in — the store outlives the build that
 * wrote it and can be edited by hand, so a stale or nonsensical entry falls back to the
 * built-in default rather than putting the UI in a state its controls cannot represent.
 */
public final class Settings {
    ...
    /** Values the Resolution / Averaging segmented controls can actually display. */
    ...
    /**
     * Restore stored values into {@code state}, then keep the store in step as they change.
     *
     * <p>Call <em>after</em> the audio devices have been enumerated: a saved live input is
     * only restored when its interface is actually present.
     */
    ...
    /** Forget every stored value and put {@code state} back to the built-in defaults. */
    public void reset(AppState state) {
        // Read the defaults off a fresh AppState rather than repeating them here, so they
        // cannot drift from the field initialisers.
        ...
        // Wipe last: if bind() has run, the assignments above have already fired their
        // listeners and written the defaults back, so clearing first would not leave the
        // store empty. An empty store also means a future change to a default is picked up
        // instead of being shadowed by a value we wrote out explicitly.
        try {
            prefs.clear();
            prefs.flush();
        } catch (BackingStoreException e) {
    ...
    private void restore(AppState st) {
        // Each property's current value is its own default, so AppState stays the single
    ...
    /**
     * Restore one source selection. A stored {@code live:<deviceId>} only survives if that
     * interface is plugged in — moving between rigs is the normal case, not an error — so a
     * missing device silently leaves the simulated preset selected rather than pointing the
     * UI at a line that cannot be opened.
     */
    private void restoreSource(AppState st, boolean isMic) {
        ...
        chan.set(saved);
        // Only the lower bound is knowable here; the channel count arrives when the line
        // opens, and MainApp clamps against it then.
    ...
    private void persist(AppState st) {
        // Calibration is user-initiated, rare, and the most expensive to redo, so it is
        // flushed the moment it changes rather than waiting for the periodic sync.
        st.splOffset.addListener((o, a, v) -> { prefs.putDouble(SPL_OFFSET, v.doubleValue()); flush(); });
        st.calRefSpl.addListener((o, a, v) -> { prefs.putDouble(CAL_REF_SPL, v.doubleValue()); flush(); });

        // The rest change often enough (slider drags) that flushing each time would be
        // wasteful; Preferences syncs periodically and on a clean exit, and MainApp flushes
        // on window close.
