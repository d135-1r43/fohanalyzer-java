...
/**
 * The analyzer plot: a resizable {@link Canvas} driven by an {@link AnimationTimer}.
 * Port of {@code src/lib/AnalyzerCanvas.svelte}; reads everything it needs from {@link AppState}.
 */
public final class AnalyzerView extends Region {

    private static final double FRAME_SEC = 0.022; // ~45 Hz, matching the original setInterval(22)
    ...
    public AnalyzerView(AppState state) {
        ...
        canvas.setOnMouseMoved(e -> { mouseX = e.getX(); mouseY = e.getY(); mouseInside = true; });
        canvas.setOnMousePressed(e -> { mouseX = e.getX(); mouseY = e.getY(); mouseInside = true; });
    ...
    private void frame(double t) {
        ...
        double plotW = W - Draw.PAD_L - Draw.PAD_R;
        double plotH = H - Draw.PAD_T - Draw.PAD_B;
        ...
        Draw.drawBackground(g, W, H);
        Draw.drawGrid(g, W, plotW, plotH);
        ...
        if (tfShown) Draw.drawTransferCurve(g, centers, sig.dispMic, sig.dispSolo, W, plotW, plotH);

        g.restore();

        if (tfShown) Draw.drawTransferLabels(g, W, plotW, plotH);
        if (state.markers.get()) {
            ...
            if (on) Draw.drawPeakMarkers(g, src, centers, plotW, plotH, W, col);
        }
        Double locate = state.locateFreq.get();
        if (locate != null) Draw.drawLocateLine(g, locate, W, plotW, plotH);
        if (mouseInside) {
            Draw.drawHoverTooltip(g, new Draw.Mouse(mouseX, mouseY, true), centers, xs,
                sig.dispMic, sig.dispSolo, W, plotW, plotH, micOn, soloOn);
        }
        Draw.drawZoneStrip(g, plotW, plotH, W, H);
    }
}
