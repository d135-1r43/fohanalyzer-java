...
/** Source selector: a button + popup of live inputs and simulated presets, plus a
 *  channel stepper for multi-channel live devices. Port of ChannelSelect.svelte. */
public final class ChannelSelect extends VBox {
    ...
    public ChannelSelect(StringProperty value, List<InputPreset> options,
        ...
        prev.setOnAction(e -> { if (chanIdx.get() > 0) chanIdx.set(chanIdx.get() - 1); });
        next.setOnAction(e -> { if (chanIdx.get() < chanCount.get() - 1) chanIdx.set(chanIdx.get() + 1); });
    ...
    private boolean isLive() { return value.get().startsWith("live:"); }
    ...
    private void showMenu(Button anchor) {
        ...
        if (!audioDevices.isEmpty()) {
            ...
            for (AudioDevice d : audioDevices) {
                ...
                mi.setOnAction(e -> { value.set(devVal); chanIdx.set(0); });
        ...
        for (InputPreset p : options) {
            ...
            mi.setOnAction(e -> { value.set(p.id()); chanIdx.set(0); });
            menu.getItems().add(mi);
        }
